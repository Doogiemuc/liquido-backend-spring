...
public class TestDataCreator implements CommandLineRunner {
	...
	public void run(String... args) throws LiquidoException {
		...
		if (props.test.recreateTestData) {
			...
			seedPollInElaborationInTeam(team1, "First poll in " + team1.getTeamName(), 6);
			seedPollInElaborationInTeam(team1, "Second poll in " + team1.getTeamName(), 3);
			seedPollInElaborationInTeam(team1, "And a third very nice poll in " + team1.getTeamName(), 3);
			seedPollInElaborationInTeam(team1, "Fourth important poll in " + team1.getTeamName(), 3);
	...
	/**
	 * Create on poll in the team with some proposals
	 *
	 * @param team the team for the poll
	 * @param pollTitle title for new poll
	 * @param numProposals how many proposals to create in the poll
	 * @return the poll in status ELABORATION
	 * @throws LiquidoException when there are no adminss in the team
	 */
	public PollModel seedPollInElaborationInTeam(@NonNull TeamModel team, @Nullable String pollTitle, int numProposals) throws LiquidoException {
		log.info("Seeding a poll with " + numProposals + " proposals in " + team.toString());
		long now = System.currentTimeMillis() % 1000;

		// fake login admin, only the admin is allowed to create several proposals in a poll
		...
		pollTitle = pollTitle != null ? pollTitle : "Poll " + now + " in Team " + team.getTeamName();
		PollModel poll = pollService.createPoll(pollTitle, getDefaultArea(), team);

		for (int i = 0; i < numProposals; i++) {
			String proposalTitle = "Proposal " + i + " in Poll '" + pollTitle + "' in Team " + team.getTeamName();
			String description = util.getLoremIpsum(100, 300);
			LawModel proposal = new LawModel(proposalTitle, description, poll.getArea());
			proposal.setIcon(util.getRandomFaIcon());
			...
			//LawModel proposal = util.createProposal("Proposal " + i + " in Team " + team.getTeamName(), util.getLoremIpsum(100, 200), getDefaultArea(), admin, 2, 1);
			poll = pollService.addProposalToPoll(proposal, poll);
		}

		util.fakeCreateAt (poll, props.daysUntilVotingStarts / 2);
		util.fakeUpdatedAt(poll, props.daysUntilVotingStarts / 2);
		return poll;
	}
	...
	public PollModel seedPollInVotingInTeam(@NonNull TeamModel team) throws LiquidoException {
		...
		authUtil.authenticateInSecurityContext(admin.getId(), team.getId(), null);  // fake login admin
		PollModel poll = this.seedPollInElaborationInTeam(team, "Poll in Voting in " + team.getTeamName(), 5);
	...
	public void seedVotes(PollModel pollInVoting, Collection<UserModel> voters, int numVotes) {
		...
		if (pollInVoting.getNumCompetingProposals() < 2)
			throw new RuntimeException("Cannot seed votes. Poll in voting must have at least two proposals.");
