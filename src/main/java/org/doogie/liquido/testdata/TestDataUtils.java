...
public class TestDataUtils {
	...
	public LawModel createProposal(String title, String description, AreaModel area, UserModel createdBy, int ageInDays, int reachedQuorumDaysAgo) {
		...
		proposal.setIcon(getRandomFaIcon());

		// Add enough supporters so that the idea becomes a proposal. Or immediately create proposal if ideas are disabled (for mobile client)
	...
	private static String[] faIcons = {"chair", "chalkboard", "chalkboard-teacher", "charging-station", "chart-area", "chart-bar",
		"magic", "magnet", "mail-bulk", "male", "map", "map-marked", "map-marked-alt", "map-marker",
		"tag", "tags", "tape", "tasks", "taxi", "teeth", "teeth-open", "temperature-high"};

	public static String getRandomFaIcon() {
		int idx = rand.nextInt(faIcons.length);
		return faIcons[idx];
	}
